...
public class BST<Key extends Comparable<Key>, Value> {
	...
	//another way to do level order tranversal
	public Iterable<Node> levelOrder2() {
...
//			StdOut.println(s + " " + st.get(s));
	...
	public BST<Key, Value> buildFromSorted(Key[] keys, Value[] vals){
		BST<Key, Value> bst = new BST<Key, Value>();
		bst.root = addToTree(keys, vals, 0, keys.length - 1);
		return bst;
	}
	
	private Node addToTree(Key[] keys, Value[] vals, int lo, int hi){
		if (lo > hi) return null;
		int mid = lo + (hi - lo)/2;
		Node node = new Node(keys[mid], vals[mid], 0);
		node.left = addToTree(keys, vals, 0, mid - 1);
		node.right = addToTree(keys, vals, mid + 1, hi);
		if (node.left == null)
			node.N = 1 + node.right.N;
		else if (node.right == null)
			node.N = 1 + node.left.N;
		else
	...
	public void degenerate(){
	    root = degenerate(root);
	}

	public Node degenerate(Node node){
	    if (node == null) return null;
	    node.left = degenerate(node.left);
	    if (node.left == null){ 
	    	node.right = degenerate(node.right);
	        return node;
	    }
	    else {
	        Node n = rotateRight(node);
	        n.right = degenerate(n.right);
	        return n;
	    }
	}
	...
	/** 
	 * another algo to degenerate a tree to right leaning linked list.
	 */
	public void flatten() {
